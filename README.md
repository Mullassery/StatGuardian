...
- **CI's `cargo audit` step is currently failing** on real RustSec advisories in transitive dependencies, including two HIGH-severity (7.5) findings in `quick-xml` — see `docs/SECURITY_AUDIT.md`'s 2026-09-11 update for the full list and why several aren't simple fixes (some need major-version bumps of direct dependencies like `pyo3`, which risk breaking this crate's own bindings).
