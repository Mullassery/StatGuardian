...
use polars::prelude::*;
use regex::Regex;
use statguardian_core::ast::{Constraint, DataType as ContractType, FieldDef, Severity};
            ...
            match df.column(&field.name) {
                ...
                Ok(col) => {
                    if let Some(series) = col.as_series() {
                        ...
                        violations.extend(check_constraints(
                            series,
                            &field.constraints,
                            &field.name,
                        ));
                ...
                if let Ok(fs) = series.cast(&DataType::Float64) {
                    if let Ok(ca) = fs.f64() {
                        let failing: Vec<usize> = ca
                            .iter()
                            .enumerate()
                            .filter_map(|(i, v)| v.filter(|&x| x < *min || x > *max).map(|_| i))
                            .collect();
                        if !failing.is_empty() {
                            violations.push(
                                Violation::new(
                                    ...
                                    "between",
                                    format!(
                                        "{} value(s) outside [{min},{max}] in '{col}'",
                                        failing.len()
                                    ),
                                    Severity::Error,
                                )
                ...
                if let Ok(fs) = series.cast(&DataType::Float64) {
                    if let Ok(ca) = fs.f64() {
                        let failing: Vec<usize> = ca
                            .iter()
                            .enumerate()
                            .filter_map(|(i, v)| v.filter(|&x| x < *value).map(|_| i))
                            .collect();
                        if !failing.is_empty() {
                            violations.push(
                                Violation::new(
                                    ...
                                    "min",
                                    format!(
                                        "{} value(s) below min={value} in '{col}'",
                                        failing.len()
                                    ),
                                    Severity::Error,
                                )
                        ...
                        if !failing.is_empty() {
                            violations.push(
                                Violation::new(
                                    ...
                                    "max",
                                    format!(
                                        "{} value(s) above max={value} in '{col}'",
                                        failing.len()
                                    ),
                                    Severity::Error,
                                )
                        ...
                        if !failing.is_empty() {
                            violations.push(
                                Violation::new(
                                    ...
                                    "regex",
                                    format!(
                                        "{} value(s) don't match '{pattern}' in '{col}'",
                                        failing.len()
                                    ),
                                    Severity::Error,
                                )
                    ...
                    if !failing.is_empty() {
                        violations.push(
                            Violation::new(
                                ...
                                "len",
                                format!(
                                    "{} value(s) with length outside [{min},{max}] in '{col}'",
                                    failing.len()
                                ),
                                Severity::Error,
                            )
