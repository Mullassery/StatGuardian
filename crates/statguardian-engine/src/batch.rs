...
use statguardian_validators::{RuleEngine, SchemaValidator, Violation};
...
impl BatchExecutor {
    pub fn execute(
        ...
        let cross_col_violations =
            RuleEngine::evaluate_cross_column(df, &contract.cross_column_rules);
...
fn execute_node(node: &DagNode, df: &DataFrame) -> Vec<Violation> {
    match node {
        ...
        DagNode::NullCheck { column, severity } => {
            let s = match get_series(df, column) {
                Some(s) => s,
                None => return vec![],
            };
            let n = s.null_count();
            ...
            if let Ok(ca) = s.str() {
                ...
                if !failing.is_empty() {
                    return vec![Violation::new(
                        ...
                        "len_check",
                        format!(
                            "{} value(s) with invalid length in '{column}'",
                            failing.len()
                        ),
                        severity.clone(),
                    )
            ...
            if let Ok(ca) = s.str() {
                ...
                if !failing.is_empty() {
                    return vec![Violation::new(
                        ...
                        "enum_check",
                        format!(
                            "{} value(s) not in allowed set in '{column}'",
                            failing.len()
                        ),
                        severity.clone(),
                    )
            ...
            if let Ok(fs) = s.cast(&DataType::Float64) {
                if let Ok(ca) = fs.f64() {
                    let failing: Vec<usize> = ca
                        .iter()
                        .enumerate()
                        .filter_map(|(i, v)| v.filter(|&x| x <= 0.0).map(|_| i))
                        .collect();
                    if !failing.is_empty() {
                        return vec![Violation::new(
                            column,
        ...
        DagNode::QualityMetricCheck {
            ...
            df,
            &[statguardian_core::ast::QualityRule {
                metric: metric.clone(),
                column: column.clone(),
                op: op.clone(),
                threshold: *threshold,
...
fn detect_outliers(
    ...
    let float_s = match s.cast(&DataType::Float64) {
        Ok(f) => f,
        Err(_) => return vec![],
    };
    ...
    let outlier_indices: Vec<usize> = match method {
        "iqr" => {
            ...
            let hi = q3 + 1.5 * iqr;
            vals.iter()
                .filter_map(|(i, x)| if *x < lo || *x > hi { Some(*i) } else { None })
                .collect()
        }
        "zscore" => {
    ...
    if outlier_indices.is_empty() {
        ...
        vec![Violation::new(
            ...
            "outlier_detection",
            format!(
                "{} outlier(s) in '{column}' (method={method})",
                outlier_indices.len()
            ),
            severity.clone(),
        )
        .with_rows(outlier_indices)]
    }
}
